...
if Hooks then
  ...
  Hooks:Add("LocalizationManagerPostInit", "LocalizationManagerPostInit_Holo", function(loc)
    ...
    if Holo.options.Holo_lang == 2 and io.open("mods/holo/loc/cn.txt") then
      loc:load_localization_file("mods/holo/loc/cn.txt")
    end
  end)
  ...
  if LuaModManager:GetNumberOfJsonKeybinds() == 0 then
    Hooks:Add("MenuManager_Base_BuildModOptionsMenu", "Voicekey_opt_build", function(menu_manager, nodes)
      ...
      MenuHelper:AddMenuItem(nodes.options,
        keybinds_menu_id,
        "base_options_menu_keybinds",
        "base_options_menu_keybinds_desc",
        "lua_mod_options_menu", "after")
    end)
  end
end
