if Holo.options.Menu_enable then
  ...
  function BlackMarketGuiTabItem:refresh()
    ...
    self._tab_panel:child("tab_select_rect"):set_color((self._selected
      or self._highlighted)
      and Holomenu_color_tab_highlight
      or Holomenu_color_tab)
  ...
  local BlackMarketGuiButtonItem = BlackMarketGuiButtonItem or class(BlackMarketGuiItem)
  ...
  function BlackMarketGuiButtonItem:init(main_panel, data, x)
    ...
    local up_font_size = NOT_WIN_32
      and RenderSettings.resolution.y < 720
      and self._data.btn == "BTN_STICK_R"
      and 2 or 0
  ...
  function BlackMarketGui:mouse_moved(o, x, y)
    ...
    if not self._highlighted then
      ...
      used = false
      pointer = "arrow"
    elseif not inside_tab_scroll
      or self._tabs[self._highlighted]
      and not self._tabs[self._highlighted]:inside(x, y) then
    ...
    if self._tabs[self._selected] then
      ...
      if tab_used then
        ...
        self._select_rect:set_visible(y > self._tabs[self._selected]._grid_panel:top()
          and y < self._tabs[self._selected]._grid_panel:bottom()
          and self._selected_slot
          and self._selected_slot._name ~= "empty")
    ...
    if self._market_bundles then
      ...
      if active_bundle then
        for key, data in pairs(active_bundle) do
          if key ~= "panel"
            and (alive(data.text)
            and data.text:inside(x, y)
            or alive(data.image)
            and data.image:inside(x, y)) then
  ...
  function BlackMarketGuiSlotItem:select(instant, no_sound)
    ...
    if self._text_in_mid and alive(self._panel:child("text")) then
      ...
      self._panel:child("text"):set_text(self._data.mid_text.no_upper
        and self._data.mid_text.selected_text
        or utf8.to_upper(self._data.mid_text.selected_text or ""))
  ...
  function BlackMarketGuiSlotItem:deselect(instant)
    ...
    if self._text_in_mid and alive(self._panel:child("text")) then
      ...
      self._panel:child("text"):set_blend_mode("normal")
      self._panel:child("text"):set_text(self._data.mid_text.no_upper
        and self._data.mid_text.noselected_text
        or utf8.to_upper(self._data.mid_text.noselected_text or ""))
