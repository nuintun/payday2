...
function Holo:CheckForHuds() 
	Holo.options.HoxHud_support = io.open("lib/Native/HoxHud.dll", "rb") ~= nil 
	Holo.options.PDTHHud_support_tm = io.open("mods/PDTH Hud/mod.txt", "rb") ~= nil
	Holo.options.PDTHHud_support = Holo.options.PDTHHud_support_tm and Holo.options.hudtop_enable 
	Holo.options.CopactHud_support = io.open("mods/CompactHUD/mod.txt", "rb") ~= nil
	Holo.options.GageHud_support = io.open("mods/GageHud/mod.txt", "rb") ~= nil 
	Holo:Save()
end
...
function Holo:init()
	...
	--Info
    hudtop_enable = Holo.options.hudtop_enable
...
function Holo:update()
    ...
    if managers.hud ~= nil then
		if hudtop_enable then
			if managers.hud._hud_assault_corner.update then
				managers.hud._hud_assault_corner:update()
			end
			if managers.hud._hud_objectives.update then
				managers.hud._hud_objectives:update()
			end
			if managers.hud._hud_heist_timer.update then
		...
		if managers.hud._teammate_panels[managers.hud.PLAYER_PANEL].update then
	    ...
	    if managers.hud._hud_hint.update then
	    	managers.hud._hud_hint:update()
	    end
	    if managers.hud._hud_presenter.update then
	   		managers.hud._hud_presenter:update()
	   	end
	   	if managers.hud._hud_temp.update then
...
if not Holo.setup then
	...
	Holo.options.Holocolor = Holo.options.Holocolor or 1
 	...
 	Holo.colors[1].color = HoloColor	
 	log("[HoloHUD] Done loading options")
 	Holo.setup = true
end
