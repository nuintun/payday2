...
if Holo.options.hudteammate_enable and (not Holo.options.PDTHHud_support_tm or not pdth_hud.Options.HUD.MainHud) and not Holo.options.GageHud_support then 
function HUDTeammate:init( ... )
	...
	if CompactHUD then
		Holo:ApplySettings({cable_ties_panel:child("bg"),deployable_bg,grenades_panel:child("bg")},{visible = false})
	else
...
if Holo.options.GageHud_support or not Holo.options.hudteammate_enable then
	function HUDTeammate:init(...)
		self.old.init(self, ...)
		if self._player_panel and self._player_panel:child("radial_health_panel") then
			local radial_health_panel = self._player_panel:child("radial_health_panel")
			radial_health_panel:child("radial_shield"):set_blend_mode("normal")
			radial_health_panel:child("radial_health"):set_blend_mode("normal")			
			radial_health_panel:child("radial_shield"):set_image("guis/textures/pd2/hud_shield")
		...
		if self._health_panel then
			...
			self._health_panel:child("radial_shield"):set_image("guis/textures/pd2/hud_shield")
			self._health_panel:child("radial_health"):set_image("guis/textures/pd2/hud_health")
		end
	end
end