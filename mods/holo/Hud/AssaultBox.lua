...
function HUDAssaultCorner:init( ... )
	...
	if Holo.options.hudtop_enable then 	
		...
		casing_icon:set_color(Casing_color)
	   	...
	   	Holo:ApplySettings({CasingBG, NoreturnBG, point_of_no_return_timer, point_of_no_return_text, noreturn_icon, casing_icon}, {blend_mode = "normal"}) 
		noreturn_icon:show()
	...
	if Holo.options.hostages_enable and Infohud_enable and (not Holo.options.PDTHHud_support_tm or not pdth_hud.Options.HUD.MainHud or Holo.options.hudtop_enable) then 	
		local hostages_icon = hostages_panel:child( "hostages_icon" )
		self._hostages_bg_box:hide(0)
		self._hostages_bg_box:set_alpha(0)		
		hostages_icon:hide()
		hostages_icon:set_alpha(0)
    elseif Holo.options.hudtop_enable and not Holo.options.HoxHud_support then
		local num_hostages = self._hostages_bg_box:child( "num_hostages" )
		local HostageBG = self._hostages_bg_box:child( "bg" )
