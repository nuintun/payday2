  ...
  if not HUDTeammate.increment_kill_count and not HUDManager.CUSTOM_TEAMMATE_PANEL then
    ...
    function HUDTeammate:_update_kill_count_text()
      ...
      if HUDTeammate.SHOW_HEADSHOT_KILLS then
        kill_string = tostring(self._headshot_kills) .. "/"
      end

      kill_string = kill_string .. tostring(self._kill_count)
...
elseif string.lower(RequiredScript) == "lib/managers/hudmanagerpd2" then
  --Custom HUD compatibility
  HUDManager._USE_KILL_COUNTER = true
...
elseif string.lower(RequiredScript) == "lib/units/enemies/cop/copdamage" then
  ...
  function CopDamage:_process_kill(aggressor, i_body)
    if alive(aggressor) and aggressor:base() then
      ...
      elseif aggressor:base()._grenade_entry then
