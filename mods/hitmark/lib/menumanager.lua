...
function EnhancedHitmarkers.EHNotificationClickCallback()
  setup:quit()
  return true
end
  ...
  if menu == "menu_main" then
    if EnhancedHitmarkers._hit_texture_overrides_not_initialized or EnhancedHitmarkers._kill_texture_overrides_not_initialized then
      if not NotificationsManager:NotificationExists("EH restart required") then
        NotificationsManager:AddNotification("EH restart required", managers.localization:text("eh_options_menu_title"), managers.localization:text("eh_options_info_popup_message_restart_required"), 1000, EnhancedHitmarkers.EHNotificationClickCallback)
      end
    end
  end
end)
...
Hooks:Add("LocalizationManagerPostInit", "LocalizationManagerPostInit_EnhancedHitmarkers", function(loc)
  if _G.PD2KR then
    loc:load_localization_file(EnhancedHitmarkers._path .. "loc/korean.txt")
  else
    for _, filename in pairs(file.GetFiles(EnhancedHitmarkers._path .. "loc/")) do
      local str = filename:match('^(.*).txt$')
      if str and Idstring(str) and Idstring(str):key() == SystemInfo:language():key() then
        loc:load_localization_file(EnhancedHitmarkers._path .. "loc/" .. filename)
        break
      end
    end
  end
